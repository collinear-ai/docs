...
The SDK validates these persona buckets. Supplying unsupported ages or occupations raises immediately, while combinations that pair `"Retired"` with an age bucket below `"35-44"` are simply skipped.
...
Inspect them in your editor or an interactive shell, then move on to the [Trait Basis simulation guide](simulations) for deeper configuration or the [Platform API](/get-started/platform-access) when you need hosted runs.
