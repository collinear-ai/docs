...
client = Client(
    ...
    collinear_api_key=TRAITMIX_API_KEY,
)
...
`client.simulation_runner.list_traits()` returns the Trait Mix catalog recognised by your key. If a run fails with “unknown trait”, inspect that list, adjust the payload, and retry.
