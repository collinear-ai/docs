---
title: Trait Mix Simulations
description: Configure trait mix payloads for large-scale conversation generation.
---

The Trait Mix API samples personas by taking the Cartesian product of the axes you supply. Keep the structure tight and you can generate millions of consistent conversations without hand-written prompts.

## Trait Mix payload fields
...
conversations = client.simulate(
    ...
    num_exchanges=4,
    batch_delay=0.1,
    max_concurrency=4,  # Switches to Trait Mix batch endpoint when >1
)
```
...
`client.simulation_runner.list_traits()` returns the current Trait Mix catalog supported by the service. If a run fails with “unknown trait”, inspect that list, adjust the payload, and retry. See the API reference: [List Supported Traits](/api-reference/endpoint/helpers).
