...
trait_basis = {
    ...
    "intents": ["billing_dispute", "update_plan"],
    "traits": {"impatience": [0, 2], "confusion": [0, 2]},
}
```

All trait intensity values live in the required 0–2 range.
...
client = Client(
    ...
    steer_api_key=client_settings.get("trait_basis_api_key")
    or os.getenv("TRAIT_BASIS_API_KEY")
    or os.getenv("STEER_API_KEY")
    or "demo-001",
...
Swap the placeholder API key for a secure value. For collaborative notebooks, rely on environment variables or the platform’s secret storage rather than embedding keys directly in the notebook.
