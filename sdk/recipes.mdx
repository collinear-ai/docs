...
for _ in range(5):
    ...
    client.simulate(steer_config, k=1, num_exchanges=6, batch_delay=0)
...
```python
for sim in client.simulate(steer_config, k=20, num_exchanges=2, progress=False):
    logger.info(
...
If the steer service rejects an unknown trait (`422 Unprocessable Entity`), call `client.simulation_runner.list_traits()` to inspect supported trait names, update your config, and retry. This mirrors the guardrail logic inside `SimulationRunner` that skips unsupported mixes in scripts such as `example_mix_traits.py`.
